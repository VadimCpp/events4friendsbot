 ...
 * @param {number|string} communityId - идентификатор сообщества для которого осуществляется поиск
 * @returns найденный объект или null
 *
 * Пример возвращаемого значения:
 * { "chatName": "events4friends", "date": null, "pinnedMessageId": null, "communityId": "1" }
 */
const getPinnedMessage = (pinnedMessages, communityId) => {
  if (pinnedMessages && pinnedMessages.length) {
    for (let i = 0; i < pinnedMessages.length; i++) {
      ...
      if (parseInt(pinnedMessage.communityId) === parseInt(communityId)) {
